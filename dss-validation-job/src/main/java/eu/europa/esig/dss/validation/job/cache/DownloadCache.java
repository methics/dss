 ...
 *
 */
public class DownloadCache extends AbstractCache<DownloadResult> {
	 ...
	 * @param downloadedResult {@link DownloadResult} value to compare with
	 * @return TRUE if digests match (file is up to date), FALSE otherwise
	 */
	public boolean isUpToDate(CacheKey cacheKey, DownloadResult downloadedResult) {
		...
		CachedEntry<DownloadResult> cachedFileEntry = get(cacheKey);
		if (!cachedFileEntry.isEmpty()) {
			DownloadResult cachedResult = cachedFileEntry.getCachedResult();
	...
	private boolean isSHA2ContentMatch(DownloadResult cachedResult, DownloadResult downloadedResult) {
